(function () {
...
})()(
  {
    ...
    5: [
      function (require, module, exports) {
        const SpooderVersion = '0.5.9';
        const OSC = require('osc-js');
        window.OSC = OSC;
        const urlParams = new URLSearchParams(window.location.search);
        const isBridgeMode = window.self !== window.top && urlParams.get('bridge') === '1';
        // The combined overlay container (/overlays/) includes this bundle only to use the
        // global OSC library directly - it drives its own multi-plugin connection in
        // container.js and must not have this module auto-connect under a bogus pluginName.
        const isContainerHost = window.location.pathname.split('/')[1] === 'overlays';

        // Neutralize the browser's default UA stylesheet (e.g. the 8px body margin) so an
        // overlay's own CSS is what determines its layout, whether it's full-viewport
        // standalone or sized/positioned inside the container's iframe.
        ...
        function initBridge() {
          ...
          window.addEventListener('message', (event) => {
            ...
            console.log("Should call onOSCOpen", data);
            ...
            switch (data.type) {
              ...
              case 'message':
                if (typeof getOSCMessage != 'undefined') {
                  getOSCMessage({ address: data.address, args: data.args });
                }
                break;
        ...
        function sendOSCStandalone(address, ...message) {
          let pMessage = null;
          if (message.length == 1) {
            pMessage = message[0];
          } else {
        ...
        function sendOSCBridge(address, ...message) {
          let pMessage = null;
          if (message.length == 1) {
            pMessage = message[0];
          } else {
          ...
          window.parent.postMessage(
            {
              ...
              pluginName: pluginName,
              address: address,
              args: Array.isArray(pMessage) ? pMessage : [pMessage],
            },
            window.location.origin,
          );
        }

        window.sendOSC = isBridgeMode ? sendOSCBridge : sendOSCStandalone;
