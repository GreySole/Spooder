...
export default class OSCServiceNew {
  private static instance: OSCServiceNew;
  private osctunnels = {} as KeyedObject;
  private oscUDP!: OscUdpServer;
  private oscTCP!: OscTcpServer;
  public static sendToTCP: (
    address: string,
    oscValue: any,
  ...
  constructor() {
    if (OSCServiceNew.instance) {
      return OSCServiceNew.instance;
    }

    OSCServiceNew.instance = this;

    try {
      const oscFilePath = userDir + '/settings/osc-tunnels.json';
      if (!fs.existsSync(oscFilePath)) {
        OSCServiceNew.saveTunnels({});
      } else {
    ...
    OSCServiceNew.sendToTCP = OSCServiceNew.instance.oscTCP.sendToTCP.bind(
      OSCServiceNew.instance.oscTCP,
    );
    OSCServiceNew.sendToUDP = OSCServiceNew.instance.oscUDP.sendToUDP.bind(
      OSCServiceNew.instance.oscUDP,
    );
  }

  static getTunnels() {
    return OSCServiceNew.instance.osctunnels;
  }
