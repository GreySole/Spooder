...
import { CoreModule, backendDir, KeyedObject } from 'src/Types.ts';
...
export class OBSOSC implements ControlModuleInterface {
  constructor(router: Router) {
    router.get('/obs/get_output_settings', (req: Request, res: Response) => {
      res.send(this.settings);
    });

    router.post('/obs/save_output_settings', (req: Request, res: Response) => {
    ...
    router.get('/obs/get_scenes', async (req: Request, res: Response) => {
      if (this.connected == false) {
        res.send({ status: 'notconnected' });
      } else {
        let obsReturn = {} as KeyedObject;
        let obsScenes: KeyedObject =
          (await this.call('GetSceneList')) ?? ({ scenes: {} } as KeyedObject);
        if (obsScenes == null) {
          res.send({});
          return;
        }
        ...
        let obsInputs: KeyedObject = (await this.call('GetInputList')) ?? ({} as KeyedObject);
  ...
  saveLogin(url: string, port: number, password: string) {
  ...
  async connect(url?: string, port?: number, password?: string) {
    if (this.connected) {
      return;
    }
    ...
    if (address[1] == 'obs') {
      if (address[2] == 'stream') {
        ...
        if (message.args[0] == 'start') {
          ...
          if (this.settings.recordRename) {
            let recordStatus = (await this.call('GetRecordStatus')) as KeyedObject;
                ...
                for (let o in objects) {
                  if (objects[o] == 'stream') {
                    ...
                    if (
                      ...
                      twitch.chat.restartChat(this.settings.disconnectAlert ? 'disconnected' : '');
                      ...
                      this.streamReconnecting = false;
                      twitch.chat.restartChat(this.settings.disconnectAlert ? 'reconnect' : '');
                    }
        ...
        if (address[3] == 'input') {
          if (address[4] == 'mute') {
            ...
            this.call('GetCurrentProgramScene').then((programScene: any) => {
              ...
              this.call('GetSceneItemList', {
                ...
                for (let item in sceneItemList) {
                  ...
                  if (sceneItemList[item].isGroup == true) {
                    let thisGroupItems = await this.call('GetGroupSceneItemList', {
            ...
            this.call('GetCurrentProgramScene').then((programScene: any) => {
              ...
              this.call('GetSceneItemList', {
                ...
                for (let item in sceneItemList) {
                  ...
                  if (sceneItemList[item].isGroup == true) {
                    finalInputList.groups[sceneItemList[item].sourceName] = await this.call(
          ...
          this.call('GetStudioModeEnabled').then((studioData: any) => {
          ...
          if (address[4] == 'list') {
            ...
            this.call('GetGroupSceneItemList', { sceneName: message.args[0] }).then(
              (sceneData: any) => {
