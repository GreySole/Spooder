...
class STwitch {
    ...
    constructor(router, publicRouter) {
        ...
        router.get("/twitch/eventsubs/list", (req, res) => {
        ...
        router.get("/twitch/config", async (req, res) => {
            ...
            sendSubs.eventsubList = this.eventsubs;
            sendSubs.scopeList = this.scopes;
            res.send(sendSubs);
        });
        ...
        router.get("/twitch/get_channelpoint_rewards", async (req, res) => {
            ...
            await Axios({
                ...
                .then((response) => {
                    ...
                    twitchLog("Channel point get error: ", error.message, error.response?.data?.message);
    ...
    callBotAPI(url, postBody, method) {
        ...
        return new Promise((res, rej) => {
            Axios({
                ...
                .catch(error => {
                    twitchLog("Bot API use error: ", error.message, error.response?.data?.message);
                    rej(error);
                });
        })
    }
    ...
    callAppAPI(url, postBody, method) {
        ...
        return new Promise((res, rej) => {
            Axios({
                ...
                .then(data => res(data.data))
                .catch(error => {
                    twitchLog("App API use error: ", error.message, error.response?.data?.message);
                    rej(error);
                });
        })
    }
    ...
    callBroadcasterAPI(url, postBody, method) {
        ...
        return new Promise((res, rej) => {
            Axios({
                ...
                .then(data => res(data.data))
                .catch(error => {
                    twitchLog("Broadcaster API use error: ", error.message, error.response?.data?.message);
                    rej(error);
                });
        })
    }
        ...
        return new Promise((res, rej) => {
            Axios({
                ...
                .then((response) => {
                    ...
                    twitchLog("Braodcaster validate error: ", error.message, error.response?.data?.message);
                    if (error.response?.status == 401) {
                        this.onBroadcasterAuthFailure().then(async newtoken => {
                            await this.validateBroadcaster();
                            res({ status: "newtoken", newtoken: newtoken });
                        }).catch(error=>twitchLog(error.response?.data?.message));
                    } else {
        ...
        return new Promise((res, rej) => {
            Axios({
                ...
                .then((response) => {
                    ...
                    twitchLog("Bot validate error: ", error.message, error.response?.data?.message);
                    if (error.response?.status == 401) {
