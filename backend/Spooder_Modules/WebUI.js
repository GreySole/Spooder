...
const motherwolf = require("./Motherwolf");
...
function isLocal(req) {
  ...
  if (isLocal == false) {
    logToFile(
      ...
      `IP: ${remoteAddress} Path: ${req.path} Cookie: ${
        req.cookies?.access != null ? "PRESENT" : "NONE"
      }`,
      300
    );
  }
...
class WebUI {
  ...
  registerPluginApi(context, router, method, address, funct) {
    if (router === "local") {
      if (method.toLowerCase() === "get") {
        this.pluginApi.local.get[path.join(context.dirname, address)] =
          funct.bind(context);
      } else if (method.toLowerCase() === "post") {
        this.pluginApi.local.post[path.join(context.dirname, address)] =
          funct.bind(context);
      }
    } else if (router === "public") {
      if (method.toLowerCase() === "get") {
        this.pluginApi.public.get[path.join(context.dirname, address)] =
          funct.bind(context);
      } else if (method.toLowerCase() === "post") {
        this.pluginApi.public.post[path.join(context.dirname, address)] =
          funct.bind(context);
      }
    } else {
      throw new Error(
        `Unknown router: ${router}. There's only local and public routers.`
      );
    }
    console.log("REGISTERED PLUGIN API", this.pluginApi);
  }
  ...
  startServer(devMode) {
    ...
    if (initMode == true) {
      ...
      publicRouter.use(
        "/settings",
        express.static(backendDir + "/web/settings")
      );
    ...
    async function pluginGet(req, res) {
      ...
      try {
        var thisPlugin = fs.readFileSync(
    ...
    router.get("/server_config", (req, res) => {
      ...
      let backups = {
        settings: fs.existsSync(backupSettingsDir)
          ? fs.readdirSync(backupSettingsDir)
          : {},
        plugins: fs.existsSync(backupPluginsDir)
          ? fs.readdirSync(backupPluginsDir)
          : {},
      };
    ...
    router.post("/saveCustomSpooder", async (req, res) => {
      ...
      fs.writeFile(
        backendDir + "/settings/themes.json",
    ...
    router.get("/reinstall_plugin", async (req, res) => {
      ...
      await this.installPluginDependencies(
        pluginName,
        path.join(backendDir, "plugins", pluginName)
      );
      this.getPlugins();
      res.send({ status: "ok" });
    });
    ...
    router.get("/checkout_settings/*", async (req, res) => {
      ...
      webLog(
        ...
        path.join(backendDir, "backup", "settings", backupName)
      );
    ...
    router.post("/checkin_settings", (req, res) => {
      if (!req.files) {
        ...
        req.files.file.mv(
          path.join(backendDir, "backup", "settings", req.files.file.name)
        );
        let newSettingsBackups = fs.readdirSync(
          path.join(backendDir, "backup", "settings")
        );
    ...
    router.post("/checkin_plugins", (req, res) => {
      if (!req.files) {
        ...
        req.files.file.mv(
          path.join(backendDir, "backup", "plugins", req.files.file.name)
        );
        let newSettingsBackups = fs.readdirSync(
          path.join(backendDir, "backup", "plugins")
        );
    ...
    router.post("/backup_settings", async (req, res) => {
      ...
      zip.writeZip(
        backendDir + "/backup/settings/" + backupName + ".zip",
        (e) => {
          if (e) {
    ...
    router.post("/backup_plugins", async (req, res) => {
      ...
      zip.writeZip(
        backendDir + "/backup/plugins/" + backupName + ".zip",
        (e) => {
          if (e) {
            throw new Error(e.message);
          }
          let newPluginBackups = fs.readdirSync(
            path.join(backendDir, "backup", "plugins")
          );
    ...
    router.post("/restore_settings", async (req, res) => {
      ...
      if (selections["everything"] == true) {
        fs.copySync(path.join(fileDir), path.join(backendDir, "settings"), {
          overwrite: true,
        });
      } else {
    ...
    router.post("/restore_plugins", async (req, res) => {
      ...
      for (let p in pluginList) {
        if (
          fs.existsSync(
            path.join(backendDir, "plugins", pluginList[p], "node_modules")
          )
        ) {
        ...
        if (
          fs.existsSync(
            path.join(backendDir, "plugins", pluginList[p], "package.json")
          )
        ) {
      ...
      for (let w in webfolders) {
        if (webfolders[w] != "mod") {
          fs.rmSync(path.join(backendDir, "web", webfolders[w]), {
            recursive: true,
          });
        }
      }
    ...
    router.post("/delete_plugin_asset", async (req, res) => {
      ...
      let thisPluginAssets =
        fs.existsSync(assetDir) == true ? fs.readdirSync(assetDir) : null;
    ...
    router.post("/get_plugin_assets", async (req, res) => {
      ...
      walk(
        path.join(backendDir, "web", "assets", pluginName),
        (err, results) => {
    ...
    router.post("/browse_plugin_assets", async (req, res) => {
      ...
      if (
        !fs.existsSync(
          path.join(backendDir, "web", "assets", pluginName, currentPath)
        )
      ) {
    ...
    router.post("/save_plugin", async (req, res) => {
      ...
      let settingsFile = path.join(
        backendDir,
        "plugins",
        newSettings.pluginName,
        "settings.json"
      );
    ...
    router.get("/plugins", async (req, res) => {
      ...
      for (let a in activePlugins) {
        ...
        pluginPacks[a] = {
          ...
          version:
            activePlugins[a].version == null
              ? "Unknown Version"
              : activePlugins[a].version,
          author:
            activePlugins[a].author == null
              ? "Unknown Author"
              : activePlugins[a].author,
          description:
            activePlugins[a].description == null
              ? ""
              : activePlugins[a].description,
          dependencies:
            activePlugins[a].dependencies == null
              ? {}
              : activePlugins[a].dependencies,
          settings: thisPlugin,
        ...
        if (
          activePlugins[a].status != null &&
          activePlugins[a].status != "ok"
        ) {
    ...
    router.get("/get_plugin/*", async (req, res) => {
      let plugin = {};
      let a = req.params["0"];
      let thisPlugin = fs.readFileSync(
        backendDir + "/plugins/" + a + "/settings.json",
        { encoding: "utf8" }
      );
    ...
    router.post("/saveUsers", async (req, res) => {
      ...
      for (let n in nameChanges) {
        ...
        if (users.trusted_users_pw[n] != null) {
          if (nameChanges[n] != null) {
            users.trusted_users_pw[nameChanges[n]] = Object.assign(
              {},
              users.trusted_users_pw[n]
            );
          }
    ...
    async function userVerify(req, res) {
      ...
      if (vType == "twitch") {
        if (Object.values(users["trusted_users"].twitch).includes(username)) {
          let sUsername = Object.keys(users["trusted_users"].twitch)[
            Object.values(users["trusted_users"].twitch).indexOf(username)
          ];
        ...
        if (Object.keys(users["trusted_users"].discord).includes(username)) {
          ...
          let response = await discord.sendInteraction(
            users["trusted_users"].discord[username],
            {
              content:
                "Hi, it looks like you're creating a login for me. If this is you, click confirm.",
          ...
          const collectorFilter = (i) =>
            i.user.id === users["trusted_users"].discord[username];
      ...
      if (users["trusted_users_pw"][username] == null) {
        ...
        if (activeUsers.pending[vusername].verified == true) {
          if (vusername !== activeUsers.pending[vusername].sUsername) {
            users["trusted_users"].permissions[vusername] =
              users["trusted_users"].permissions[
                activeUsers.pending[vusername].sUsername
              ].slice();
            users["trusted_users"].twitch[vusername] =
              users["trusted_users"].twitch[
                activeUsers.pending[vusername].sUsername
              ].slice();
            users["trusted_users"].discord[vusername] =
              users["trusted_users"].discord[
                activeUsers.pending[vusername].sUsername
              ].slice();
            delete users["trusted_users"].permissions[
              activeUsers.pending[vusername].sUsername
            ];
            delete users["trusted_users"].twitch[
              activeUsers.pending[vusername].sUsername
            ];
            delete users["trusted_users"].discord[
              activeUsers.pending[vusername].sUsername
            ];
          }
        ...
        if (
          crypto
            .pbkdf2Sync(
              password,
              users["trusted_users_pw"][username].salt,
              1000,
              64,
              `sha512`
            )
            .toString(`hex`) === users["trusted_users_pw"][username].hash
        ) {
    ...
    async function modUtil(req, res) {
      ...
      let modplugins = {};
      for (let p in activePlugins) {
        let hasUtility = fs.existsSync(
          path.join(backendDir, "web", "utility", p)
        );
        modplugins[p] = {
          name: p,
    ...
    router.post("/plugin/*", (req, res) => {
      console.log("GOT POST", req.params[0]);
  ...
  async refreshPlugin(pluginName) {
    try {
      ...
      delete require.cache[
        require.resolve(backendDir + "/plugins/" + pluginName)
      ];
      try {
        activePlugins[pluginName] = new (require(backendDir +
          "/plugins/" +
          pluginName))();
        if (
          fs.existsSync(backendDir + "/plugins/" + pluginName + "/package.json")
        ) {
          let pluginMeta = JSON.parse(
            fs.readFileSync(
              backendDir + "/plugins/" + pluginName + "/package.json",
              { encoding: "utf8" }
            )
          );
        ...
        if (
          ...
          activePlugins[pluginName].settings = JSON.parse(
            fs.readFileSync(
              backendDir + "/plugins/" + pluginName + "/settings.json",
              { encoding: "utf8" }
            )
          ...
          if (activePlugins[pluginName].onSettings != null) {
            activePlugins[pluginName].onSettings(
              activePlugins[pluginName].settings
            );
          }
        }
      } catch (e) {
  ...
  async getPlugins() {
    try {
      ...
      for await (const dirent of dir) {
        delete require.cache[
          require.resolve(backendDir + "/plugins/" + dirent.name)
        ];
        try {
          activePlugins[dirent.name] = new (require(backendDir +
            "/plugins/" +
            dirent.name))();
        } catch (e) {
          let pluginMeta = JSON.parse(
            fs.readFileSync(
              backendDir + "/plugins/" + dirent.name + "/package.json",
              { encoding: "utf8" }
            )
          );
        ...
        if (
          fs.existsSync(
            backendDir + "/plugins/" + dirent.name + "/settings.json"
          )
        ) {
          activePlugins[dirent.name].settings = JSON.parse(
            fs.readFileSync(
              backendDir + "/plugins/" + dirent.name + "/settings.json",
              { encoding: "utf8" }
            )
          ...
          if (activePlugins[dirent.name].onSettings != null) {
            activePlugins[dirent.name].onSettings(
              activePlugins[dirent.name].settings
            );
          }
        }
      }
  ...
  async installPluginFromTemp(pluginDirName, options) {
    ...
    if (!fs.existsSync(tempDir + "/command")) {
        ...
        if (fs.existsSync(tempDir + "/command/package.json")) {
          try {
            let thisPackage = JSON.parse(
              fs.readFileSync(tempDir + "/command/package.json", {
                encoding: "utf-8",
              })
            );
      ...
      if (fs.existsSync(path.join(tempDir + "/command", "node_modules"))) {
        fs.rmSync(path.join(tempDir + "/command", "node_modules"), {
          recursive: true,
        });
      }
  ...
  async startMotherwolf() {
    motherwolf.startTunnels(
      sconfig.network["mw_subdomain"],
      sconfig.network["mw_token"],
      sconfig.network.host,
      sconfig.network.host_port,
      sconfig.network.osc_tcp_port
    );
  }

  stopMotherwolf() {
    motherwolf.stopTunnels();
  }
    ...
    if (isEnabled) {
      ...
      if (shares[shareUser].discordId != null) {
          ...
          if (activePlugins[sharedPlugins[p]].hasOverlay) {
            sharedPluginMessage.push(
              activePlugins[sharedPlugins[p]].name +
                ": " +
                path.join(
                  sconfig.network.external_http_url,
                  "overlay",
                  sharedPlugins[p]
                ) +
                "?channel=" +
        ...
        if (sharedPluginMessage.length > 0) {
          discord.findUser(shares[shareUser].discordId).then((user) => {
            user.send(
              sconfig.bot.owner_name +
