...
class OBSOSC {
    ...
    router.get("/obs/get_scenes", async (req, res) => {
      if (this.connected == false) {
        ...
        for (let s in obsScenes.scenes) {
          obsReturn.scenes[obsScenes.scenes[s].sceneIndex] =
            obsScenes.scenes[s];
        }
        obsReturn.sceneItems = {};

        for (let s in obsReturn.scenes) {
          let sceneItems = await this.call("GetSceneItemList", {
            sceneName: obsReturn.scenes[s].sceneName,
          }).then((data) => data.sceneItems);
          obsReturn.sceneItems[s] = {};
          for (let si in sceneItems) {
            obsReturn.sceneItems[s][sceneItems[si].sceneItemId] =
              sceneItems[si];
          }
        }
    ...
    if (fs.existsSync(backendDir + "/settings/obs.json")) {
      try {
        this.settings = JSON.parse(
          fs.readFileSync(backendDir + "/settings/obs.json", {
            encoding: "utf-8",
          })
        );
      } catch (e) {
        console.log(
          "Somethings wrong with obs login file. Try entering it again."
        );
        this.settings = {};
      }
    }
  ...
  async connect(url, port, password) {
    ...
    try {
      ...
      obsClient.on("SceneItemEnableStateChanged", (data) => {
        sendToTCP(
          "/obs/event/SceneItemEnableStateChanged",
          JSON.stringify(data)
        );
      });
  ...
  setRecordingNameToStream() {
    return new Promise(async (res, rej) => {
      let defaultFileName = await this.call("GetProfileParameter", {
      ...
      splitTitle = splitTitle.replaceAll(
        /[`!@#$%^&*()_+\-=\[\]{};':"\\|,.<>\/?~]/g,
        ""
      );

      let recordingTitle =
        defaultFileName.defaultParameterValue.split(" ")[0] + "_" + splitTitle;
  ...
  async onOSC(message) {
    ...
    if (message.address == "/chatbox/input") {
      this.call("SendStreamCaption", { captionText: message.args[0] });
    }
      ...
      if (address[2] == "stream") {
            ...
            if (this.statusInterval == null) {
              this.statusInterval = setInterval(async () => {
                ...
                for (let o in objects) {
                  if (objects[o] == "stream") {
                    finalStatusObj[objects[o]] = await this.call(
                      "GetStreamStatus"
                    );
                    if (
                      ...
                      twitch.restartChat(
                        this.settings.disconnectAlert ? "disconnected" : null
                      );
                      ...
                      this.streamReconnecting = false;
                      twitch.restartChat(
                        this.settings.disconnectAlert ? "reconnect" : null
                      );
                    }
                    ...
                    if (this.settings.frameDropAlert) {
                      if (this.streamReconnecting == false) {
                        if (
                          finalStatusObj[objects[o]].outputSkippedFrames >
                          this.skippedFrames
                        ) {
                          this.skippedFrames =
                            finalStatusObj[objects[o]].outputSkippedFrames;
        ...
        if (address[3] == "input") {
          if (address[4] == "mute") {
            ...
            this.call("GetInputList").then((data) => {
            ...
            this.call("GetCurrentProgramScene").then((programScene) => {
              finalInputList.currentProgramSceneName =
              ...
              this.call("GetSceneItemList", {
                ...
                for (let item in sceneItemList) {
                  ...
                  let volumeData = await this.call("GetInputVolume", {
                    inputName: sceneItemList[item].sourceName,
                  }).catch((e) => {});
                  let volumeMuteData = await this.call("GetInputMute", {
                    inputName: sceneItemList[item].sourceName,
                  }).catch((e) => {});
                  if (volumeData != null) {
                    finalInputList.items[
                      sceneItemList[item].sceneItemId
                  ...
                  if (sceneItemList[item].isGroup == true) {
                    let thisGroupItems = await this.call(
                      "GetGroupSceneItemList",
                      { sceneName: sceneItemList[item].sourceName }
                    ).then((groupItemData) => groupItemData.sceneItems);
                    finalInputList.groups[sceneItemList[item].sourceName] =
                      thisGroupItems;
                    for (let gi in thisGroupItems) {
                      let thisVolumeData = await this.call("GetInputVolume", {
                        inputName: thisGroupItems[gi].sourceName,
                      }).catch((e) => {});
                      let thisVolumeMuteData = await this.call("GetInputMute", {
                        inputName: thisGroupItems[gi].sourceName,
                      }).catch((e) => {});
                      finalInputList.items[
                        gi + thisGroupItems[gi].sceneItemId
                      ] = {
                ...
                sendToTCP(
                  "/obs/get/input/volumelist",
                  JSON.stringify(finalInputList)
                );
              });
            });
          }
          ...
          if (address[4] == "list") {
            this.call("GetSceneList").then((sceneData) => {
        ...
        if (address[3] == "input") {
          ...
          this.call("SetStudioModeEnabled", {
            studioModeEnabled: message.args[0],
          });
