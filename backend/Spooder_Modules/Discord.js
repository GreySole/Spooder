...
class SDiscord {
  constructor(router) {
    ...
    this.config = fs.existsSync(backendDir + "/settings/discord.json")
      ? JSON.parse(
          fs.readFileSync(backendDir + "/settings/discord.json", {
            encoding: "utf-8",
          })
        )
      : {
  ...
  async getCommands() {
    ...
    for (let p in activePlugins) {
      if (activePlugins[p].dSlashCommands != null) {
        for (let d in activePlugins[p].dSlashCommands) {
          ...
          for (let o in activePlugins[p].dSlashCommands[d].options) {
            if (isNaN(activePlugins[p].dSlashCommands[d].options[o].type)) {
              if (
                Object.keys(this.slashCommandTypes).includes(
                  activePlugins[p].dSlashCommands[d].options[o].type
                )
              ) {
                activePlugins[p].dSlashCommands[d].options[o].type =
                  this.slashCommandTypes[
    ...
    if (this.commands.size > 0) {
      ...
      const data = await rest.put(
        Routes.applicationCommands(discordInfo.clientId),
        { body: this.commands }
      );
    ...
    return new Promise((res, rej) => {
      ...
      if (initMode == false) {
        ...
        client.on(Events.MessageReactionAdd, async (reaction) => {
          this.callPlugins("reaction", reaction);
        });
      }
      client.login(token);
    });
          ...
          if (shares[command?.[2]] != null) {
            ...
            if (command?.[3] == "start") {
              ...
              message.reply(
                command[2] +
                  "'s share is " +
                  (isSharing == true ? "running" : "not running") +
                  "."
              );
            }
          } else {
